...
def _windows_raw_hwnd(window):
    """(user32, hwnd) using window.winfo_id() directly, with NO
    GetParent() walk - the opposite case from _windows_dwm_handle()
    above. That walk exists to get past Tk's own client window to the
    OS-decorated FRAME wrapping it (titlebar, borders - what DWM
    actually draws for an ordinary window). A Toplevel built with
    overrideredirect(True) (see GUIWizard's Windows-only users-panel
    overlay) has no such decorations at all, so Windows never creates
    a separate wrapper frame for it in the first place - Tk's own
    client HWND already IS the real top-level as far as Windows is
    concerned, and GetParent() on it would just walk to whatever
    OWNS it (its Tk master, e.g. root) instead - the wrong window
    entirely for this."""
    try:
        return ctypes.windll.user32, window.winfo_id()
    except (AttributeError, OSError, tk.TclError):
        return None


def animate_window_slide(window, showing, duration_ms=200, reverse=False):
    """Windows only, no-op (returns False) elsewhere or on any failure -
    best-effort like every other Windows call in this module: a False
    here means the caller should just show/hide `window` plainly
    instead, not treat it as an error.

    Wraps user32's AnimateWindow(AW_SLIDE) - a genuinely different kind
    of tool than everything else in gui.py's own panel-toggle code
    (_animate_root_width()/_animate_log_scrim(), both hand-rolled
    Python step()/after() loops). AnimateWindow asks DWM to reveal or
    conceal an ALREADY-FULLY-RENDERED window via a smooth slide,
    entirely compositor-side - the app is never asked to redraw at any
    intermediate frame, which is exactly the cost that made every
    earlier attempt at this (a real multi-step root.geometry() glide,
    and separately a place()-based scrim wipe over live content) run
    into real, measured choppiness on Windows specifically. See
    GUIWizard's own Windows-only users-panel overlay - a separate
    borderless Toplevel, not content packed inside the main window -
    for why a SEPARATE window was necessary to use this at all:
    AnimateWindow only animates a window's own show/hide transition,
    never an in-place resize of an already-visible one, so there was
    no way to apply it to the main window directly.

    Uses _windows_raw_hwnd(), not _windows_dwm_handle() - `window` here
    is always an overrideredirect(True) Toplevel with no OS-drawn
    frame to walk up to (see that function's own docstring).

    Synchronous - MSDN: "the function will not return until the
    animation... has completed" - so this call itself blocks for
    roughly duration_ms. That's deliberate here, not a bug: it replaces
    an entire step()/after() callback chain with one call, and the sole
    reason to touch this window in the first place is to be watching
    that exact transition play out, so there's nothing productive the
    event loop needs to interleave with it.

    reverse=True flips the slide's horizontal direction - showing an
    already-open panel gliding out is the mirror image of closing it,
    not a repeat of the same wipe."""
    handle = _windows_raw_hwnd(window)
    if handle is None:
        return False
    user32, hwnd = handle
    ...
    AW_HIDE = 0x00010000
    AW_SLIDE = 0x00040000
    direction = AW_HOR_NEGATIVE if reverse else AW_HOR_POSITIVE
    flags = AW_SLIDE | direction | (0 if showing else AW_HIDE)
    try:
        return bool(user32.AnimateWindow(hwnd, ctypes.c_int(duration_ms), ctypes.c_ulong(flags)))
    except OSError:
        return False
