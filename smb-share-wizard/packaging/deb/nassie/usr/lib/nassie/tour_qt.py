...
class GuiTourQt:
    ...
    def refresh_position(self):
        ...
        if self._tracker is not None:
            self._reposition()
            # Still reported live even with the immediate call above: a
            # QTreeWidget Stretch-mode column (the shares_tree's own
            # "Path" column, which the add-row spans) recomputes its
            # width on a Qt-internal deferral, not synchronously with
            # the window's own setGeometry() - so visualItemRect() here
            # can still measure the OLD column width for one more tick
            # even though the window itself has already settled at its
            # final size. Same "the thing we're measuring hasn't caught
            # up yet" race as window_corners.py's own 50ms/250ms reapply
            # timers, just inside Qt's layout engine instead of a WM -
            # same fix shape: reposition again shortly after, once
            # that's had a real chance to run.
